/**
  ...
  * any form.
  *
  * Source:		$File: //depot/3001_TIP_NEW/TA_BASE/transactive/core/threads/src/ThreadPoolManager.cpp $
  * @author:	HoaVu
  * @version $Revision: #3 $
  *
  * Last modification : $DateTime: 2013/10/11 15:28:11 $
		...
		if ( false == m_workItemList.empty() )
		{
			...
			while ( workItemItr != m_workItemList.end() )
			{
				...
				if ( 0 != threadedWorker )
				{
                    ...
                    LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "ThreadPoolManager::processWorkItem(): Total number of queued work items = %d", m_workItemList.size() );
				}
				else
				{
