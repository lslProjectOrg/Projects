 ...
 * OCI_Initialize
 * --------------------------------------------------------------------------------------------- */
//char g_szConInitError[OCI_SIZE_BUFFER + 1];

int OCI_API OCI_Get_ErrorStrBuffer
(
OCI_Connection* pcon,
char **ppErrorStr
)
{
	...
	if (NULL != ppErrorStr)
	{
		...
		if (NULL != pcon && NULL != pcon->m_pszErrorStr)
		{
			nErrorCode = pcon->m_nErrorNode;
			*ppErrorStr = mtsdup((char*)pcon->m_pszErrorStr);
			
			pcon->m_nErrorNode = 0;
			OCI_FREE(pcon->m_pszErrorStr);
		}
	}
...
//////////////////////////////////////////////////////////////////////////
char g_szOcilibErrorBuff[OCI_SIZE_BUFFER + 1];
//extern char g_szOcilibErrorBuff[OCI_SIZE_BUFFER + 1];

OCI_EXPORT char * OCI_API OCI_Get_G_ErrorBuff
(
 void
 )
 {
	 return g_szOcilibErrorBuff;
 }
//////////////////////////////////////////////////////////////////////////
