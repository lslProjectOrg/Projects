 ...
 * OCI_ExceptionOCI
 * --------------------------------------------------------------------------------------------- */
extern char g_szOcilibErrorBuff[OCI_SIZE_BUFFER + 1];
...
)
{
    ...
    if (err != NULL)
    {
        ...
        if (NULL != con)
        {
			OCI_FREE(con->m_pszErrorStr);
			con->m_nErrorNode = err->ocode;
			con->m_pszErrorStr = mtsdup((char*)ostr);           
        }
         
		//printf("con->m_nErrorNode=[%d], con->m_pszErrorStr=[%s]\n",	con->m_nErrorNode, con->m_pszErrorStr);
				
		memset(g_szOcilibErrorBuff, 0, OCI_SIZE_BUFFER);
		sprintf(g_szOcilibErrorBuff, "Errcode=[%d],ErrStr=[%s]", (int)err->ocode, (char*)ostr);	
		//sprintf(g_szOcilibErrorBuff, "Errcode=[%d]", (int)err->ocode);	
