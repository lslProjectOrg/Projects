...
big_int CRField::getBigInt() 
{
	big_int nBigIntValue = 0;
	double fDoubleValue = 0.0;
	uword sign = OCI_NUMBER_SIGNED;//OCI_NUMBER_SIGNED; OCI_NUMBER_UNSIGNED
	OCINumber* pOCINumber = NULL;
	sword res = OCI_SUCCESS;
	...
	{
	case dtInt64:
		pOCINumber = (OCINumber*)malloc(sizeof(OCINumber));
		memset(pOCINumber, 0, sizeof(OCINumber));
		getData(pOCINumber);


		res = OCINumberToInt(OraEnvironment::getOCIError(), pOCINumber, sizeof(nBigIntValue), sign, &nBigIntValue);

		free(pOCINumber);
		pOCINumber = NULL;

		//check OCINumberToInt Res
		if (OCI_SUCCESS != res)
		{
			check(res);
		}
	...
	case dtFloat:
	case dtDouble:
		fDoubleValue = getDouble();
		nBigIntValue = (big_int)fDoubleValue;
		break;
	default:
		throwError(SCannotConvert, "big_int");
	}
...
char* CRField::getChars(char* result) {
  ...
  switch (mDataType) {
    ...
    case dtInt64:
       getChars(mBuf, sizeof(mBuf));
	   sprintf(result, "%s", mBuf);
	   break;
...
*/
    ...
    case dtInteger:
	  pBuf = (char*)malloc(nBufSize * sizeof(char));
	  memset(pBuf, 0, nBufSize * sizeof(char));
#ifdef WIN32
      itoa(getInt(), pBuf, 10);
#else
      sprintf(pBuf, "%d", getInt());
#endif      
      strncpy(result, pBuf, length);
	...
	case dtInt64://modifybylin
		...
		//format string
		sprintf(pBuf, "%s", "FM99999999999999999999999999999999999990.999999999999999999999999");
		//format string size
		nFmtBufSize = strlen(pBuf);
		//reset result buffer
		memset(result, 0, length);
		nResultBufSize = length;
		//get value
		res = OCINumberToText(OraEnvironment::getOCIError(), (OCINumber *) pOCINumber,
			(oratext *) pBuf, (ub4) nFmtBufSize,
			(oratext *) NULL, (ub4) 0,
			(ub4 *)&nResultBufSize, (oratext *)result);


		free(pOCINumber);
		pOCINumber = NULL;

		//if the end char of result is '.' or ',' replace with '/0'
		nPosTmp = -1;
		nPosTmp = (nResultBufSize / (int) sizeof(oratext)) - 1;
		if (nPosTmp >= 0)
		{
			if (',' == result[nPosTmp] || '.' == result[nPosTmp])
			{
				result[nPosTmp] = 0;
			}
		}
...
{
	char* pBuf = (char*)malloc(mRecordSize);
	memset(pBuf, 0, mRecordSize);
	return pBuf;
}
