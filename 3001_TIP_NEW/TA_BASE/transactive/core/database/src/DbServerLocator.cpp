...
namespace TA_Base_Core
{
	...
	*/
	DbServerLocator::~DbServerLocator()
	{
		...
		LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, "Destroying connections" );
	...
	*/
	bool DbServerLocator::createConnection( const std::string& connectionStr, bool isAutoReconnect )
	{
		std::vector<std::string> connDetails;
		parseConnectStr( connectionStr, connDetails, ':' );

		TA_ASSERT( CONN_STR_PARAM_COUNT == connDetails.size(), "Invalid connection string" );//size = 5

		std::string connectionStrTmp;
		std::string strDbTypeTmp;
		std::string databaseNameTmp;
		std::string userNameTmp;
		std::string passwordTmp;
		std::string hostNameTmp;
		bool isAutoReconnectTmp = false;

		connectionStrTmp = connectionStr;
		strDbTypeTmp = connDetails[0];
		databaseNameTmp = connDetails[1];
		userNameTmp = connDetails[2];
		passwordTmp = connDetails[3];
		hostNameTmp = connDetails[4];
		isAutoReconnectTmp = isAutoReconnect;

		return createConnection( connectionStrTmp, 
			strDbTypeTmp, databaseNameTmp,
			userNameTmp, passwordTmp, hostNameTmp,
			isAutoReconnect );
		//return createConnection( connectionStr, connDetails[0], connDetails[1], connDetails[2], connDetails[3], connDetails[4], isAutoReconnect );
	}
	...
	*/
		...
		bool isAutoReconnect )
	{
		...
		try
		{
			connection = createDbConnection( connectionStr, strDbType, databaseName, userName, password, hostName );
	...
	bool DbServerLocator::isOpen( const std::string& connectionStr )
	{
		...
		catch (const DBException& e)
		{
			...
			LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "DbServerLocator::isOpen():the DB connection string is: %s, Reason: %s" , 
				connectionStr.c_str(), e.getReason() );
			if (connection)
			{
				connection->decrementExecutionCount();
			}		
		...
		catch(...)
		{
			LOG_GENERIC( SourceInfo, DebugUtil::DebugError, 
				"DbServerLocator::isOpen(): unknown exception occurred, the DB connection string is: %s" , 
				connectionStr.c_str() );
			if (connection)
			{
				connection->decrementExecutionCount();
	...
	*/
	...
	{ 
		...
		{
			LOG_GENERIC( SourceInfo, DebugUtil::DebugWarn, "Tried to close an invalid connection:%s", connectionStr.c_str() );
	...
	*/
	void DbServerLocator::closeAll( const std::string& connectionStr )
	{
		if ( !connectionExists(connectionStr) )
		{
			LOG_GENERIC( SourceInfo, DebugUtil::DebugWarn, "Tried to close an invalid connection: %s",
				connectionStr.c_str() );
			return;
		}
	...
	*/
	...
	IQueryAdapter* DbServerLocator::exec( const std::string& connectionStr, const SQLStatement& rSqlObj, int numRows )
	{
		ThreadGuard guard( m_lock );
	...
	*/
	void DbServerLocator::refreshConnections()
	{
		...
		while ( it != m_connections.end() )
		{
			try
			{
				LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "Refreshing connections: %s",  it->first.c_str());
			...
			catch( const DBException& )
			{
				...
				LOG_GENERIC( SourceInfo, DebugUtil::DebugError,
					"Failed to refresh connection: %s   Keeping old connection", it->first.c_str() );
			}
	...
	int DbServerLocator::_ParseDBType(const std::string& strDbType)
	{
		...
		{    // should never reach here, if reach here, then raise a exception
			LOG_GENERIC( SourceInfo, 
				...
				"the DB type string in database config file wrong(should be: Oracle or Mysql or Sqlite): %s", strDbType.c_str() );
