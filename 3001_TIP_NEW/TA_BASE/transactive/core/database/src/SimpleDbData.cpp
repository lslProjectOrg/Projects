/**
  ...
  * in any form.
  *
  * Source: $File: //depot/3001_TIP_NEW/TA_BASE/transactive/core/data_access_interface/src/SimpleDbData.cpp $
  * @author Nick Jardine
  * @version $Revision: #9 $
  * Last modification: $DateTime: 2012/04/24 15:57:48 $
...
namespace TA_Base_Core
{
	...
	unsigned long SimpleDbData::getUnsignedLongData(const unsigned long rowIndex, const std::string& fieldName, const unsigned long defaultValue)
	{
		...
		//fix bug, strtoul support unsigned long MAX value, atol just support int MAX value
		//unsigned long value = atol(target.c_str());	
		unsigned long value = strtoul(target.c_str(), 0, 10);
	...
	time_t SimpleDbData::getDateData(const unsigned long rowIndex, const std::string& fieldName, const time_t defaultValue )
	{
        ...
        if (0 == target.size() || -1 == target.find_first_not_of("0"))
        {
			/*0000-00-00 00:00:00*/
            return defaultValue;
        }
		...
		if ((14 != target.size()) || (-1 != numberTest))
		{
			char reasonMessage[256];
	...
	double SimpleDbData::getRealData(const unsigned long rowIndex, const std::string& fieldName, const double defaultValue)
	{
		...
		int nCharE = target.find_first_of("eE");
        // PDV: end change

		int validCharacters = target.find_first_not_of("0123456789-.eE");				// locates any invalid characters in this string
		...
		else if (firstDash != otherDash && -1 != otherDash && (nCharE+1) != otherDash) // there is more than one dash and this dash not after e
		{
			char reasonMessage[256];
		...
		else if (-1 != firstDash && 0 != firstDash && (nCharE+1) != firstDash) // the only dash is not in the first location  and this dash not after e
		{
			char reasonMessage[256];
		...
		else if (-1 != firstPoint && ((0 == firstPoint) || (-1 != firstDash && 1 == firstPoint && (nCharE+1) != firstDash))) // the decimal point is in an ivalid location (either the first position, or the second if there is a dash)
		{
			char reasonMessage[256];
