...
CTreeItemExchange::CTreeItemExchange(const QList<QVariant> &data, CTreeItemExchange *parent /*= 0*/ )
{
	...
	m_ItemData = data;
	m_nDataTypeExchange = DataType_InstrumentCode;
}
...
void CTreeItemExchange::_AppendChildExchangeName( const CExchangeInfo* pExchangeInfo )
{
	...
	pTreeItemExchangeName->setDataType(DataType_ExchangeName);

	this->setDataType(DataType_Root);
...
void CTreeItemExchange::appendChildUnderlyingCode( const CExchangeInfo* pExchangeInfo )
{
	...
	pTreeItemUnderlyingCode->setDataType(DataType_UnderlyingCode);

	this->setDataType(DataType_ExchangeName);
...
{	
	...
	pTreeItemInstrumentCode->setDataType(DataType_InstrumentCode);

	this->setDataType(DataType_UnderlyingCode);
	this->appendChild(strItemValue, &pTreeItemInstrumentCode);
	pTreeItemInstrumentCode = NULL;

}

bool CTreeItemExchange::removeChildren(int position)
{
	int nIndex = 0;
	CTreeItemExchange* pTreeItemTmp = NULL;
	QMap<QString,CTreeItemExchange*>::iterator iterMap;
...
void CTreeItemExchange::setDataType( enDataTypeExchange nDataTypeExchange )
{
	m_nDataTypeExchange = nDataTypeExchange;
}

TA_App_App::enDataTypeExchange CTreeItemExchange::getDataType()
{
