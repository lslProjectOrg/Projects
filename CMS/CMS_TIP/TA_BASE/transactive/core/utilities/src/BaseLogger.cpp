...
{
	TA_Base_Core::CBaseLogger::getInstance();
	std::string strLogFile = "logFile.log";
	TA_Base_Core::CBaseLogger::getInstance().setlogFile(strLogFile);
	TA_Base_Core::CBaseLogger::getInstance().setlogLevel(TA_Base_Core::DebugTrace);

	_BaseLog(BaseLogSourceFLInfo, TA_Base_Core::DebugTrace, "test log main");


	TA_Base_Core::CBaseLogger::removeInstance();
}
