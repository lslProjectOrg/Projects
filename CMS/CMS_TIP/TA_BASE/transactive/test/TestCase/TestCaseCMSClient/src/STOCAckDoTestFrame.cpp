...
STOCAckDoTestFrame::STOCAckDoTestFrame(void)
{
	BOOST_LOG_FUNCTION();

	m_nFrameType_SampleValue = S_TO_C_ACK_DO_TEST;
	m_nDoTestType_SampleValue = DoTestType_TestStable;
	m_strAckInfo_SampleValue = "Server Ack Client's Test Stable";

	m_id_SampleValue = 4;
	m_securityType_SampleValue = 'T';
...
int STOCAckDoTestFrame::setSampleValue()
{
	int nFunRes = 0;
...
int STOCAckDoTestFrame::setDataWithMessage(cms::Message::Ptr pMessage)
{
	int nFunRes = 0;
...
int STOCAckDoTestFrame::checkValue()
{
	int nFunRes = 0;
...
cms::Message::Ptr STOCAckDoTestFrame::getMessage()
{
...
void STOCAckDoTestFrame::logFrameInfo()
{
	LOG_TRACE<<"FrameInfo:"
		<<" "<<"m_nFrameType="<<m_nFrameType
		<<" "<<"m_nDoTestType="<<m_nDoTestType
		<<" "<<"m_strAckInfo="<<m_strAckInfo
		<<" "<<"m_id="<<m_id
		<<" "<<"m_securityType="<<m_securityType
		<<" "<<"m_code="<<m_code
		<<" "<<"m_symbol="<<m_symbol
		<<" "<<"m_securityExchange="<<m_securityExchange
		<<" "<<"m_tickSize="<<m_tickSize
		<<" "<<"m_tickValue="<<m_tickValue
		<<" "<<"m_decimalPlace="<<m_decimalPlace
		<<" "<<"m_maturityDate="<<m_maturityDate;

}

std::string STOCAckDoTestFrame::_InitMaxLenString()
{
	BOOST_LOG_FUNCTION();
